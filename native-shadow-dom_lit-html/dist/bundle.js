...
</section>`;return template}connectedCallback(){this._render();this.$input=this._root.querySelector("todo-input");this.$input.addEventListener("onSubmit",this.addItem.bind(this))}addItem(e){this._list=[...this._list,{id:this._list.length,text:e.detail,checked:false}];this._render()}removeItem(e){this._list=[...this._list.slice(0,e.detail),...this._list.slice(e.detail+1)];this._render()}toggleItem(e){const item=this._list[e.detail];this._list[e.detail]=Object.assign({},item,{checked:!item.checked});this._render()}_render(){render(this.template,this._root)}}window.customElements.define("my-todo",MyTodo);class TodoInput extends HTMLElement{constructor(){super();this._root=this.attachShadow({mode:"open"});this.state={value:""}}get template(){const template=html$1`
<style>
    #new-todo-form {
        position: relative;
        font-size: 24px;
...
</form>`;return template}handleInput(e){this.state={value:e.target.value}}handleSubmit(e){e.preventDefault();if(!this.state.value)return;this.dispatchEvent(new CustomEvent("onSubmit",{detail:this.state.value}));this.$input=this._root.querySelector("#new-todo");this.$input.value="";this.$input.blur()}connectedCallback(){render(this.template,this._root)}}window.customElements.define("todo-input",TodoInput);class TodoItem extends HTMLElement{constructor(){super();this._root=this.attachShadow({mode:"open"});this._checked=false;this._text=""}get template(){const template=html$1`
<style>
    :host {
        display: block;
    }
...
</li>`;return template}handleOnRemoved(e){this.dispatchEvent(new CustomEvent("removed",{detail:this.index}))}handleOnChecked(e){this.dispatchEvent(new CustomEvent("checked",{detail:this.index}))}connectedCallback(){setTimeout(()=>{render(this.template,this._root)},0)}}window.customElements.define("todo-item",TodoItem);