...
</section>`}connectedCallback(){this._render();this.$input=this._root.querySelector("todo-input");this.$input.addEventListener("onSubmit",this.addItem.bind(this))}addItem(e){this._list=[...this._list,{id:this._list.length,text:e.detail,checked:false}];this._render()}removeItem(e){this._list=[...this._list.slice(0,e.detail),...this._list.slice(e.detail+1)];this._render()}toggleItem(e){const item=this._list[e.detail];this._list[e.detail]=Object.assign({},item,{checked:!item.checked});this._render()}_render(){render(this.render(),this._root)}}window.customElements.define("my-todo",MyTodo);class TodoInput extends HTMLElement{constructor(){super();this._root=this.attachShadow({mode:"open"});this.state={value:""};this._handleSubmit=(e=>this.handleSubmit(e))}render(){return html$1`
<style>
    #new-todo-form {
        position: relative;
        font-size: 24px;
